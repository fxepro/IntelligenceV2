...
export default function DomainNamesSourcesPage() {
  ...
  const fetchSources = useCallback(async () => {
    setLoadError(null);
    try {
      const res = await fetch(`${API_BASE}/api/v1/sources?domain=domain_names`);
  ...
  }, [sources, searchQuery, tagFilter, priorityFilter, statusFilter]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <GovernmentSourcesTable
        ...
        title="Domain sources"
        icon="globe"
        detailBasePath="/domain-names/sources"
        emptyMessage="No domain sources yet. Seed the catalog to populate this list."
      />
    </div>
  );
}
