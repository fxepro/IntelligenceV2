...
import { sourceTypeLabel, formatFileSize, pipelineStatusLabel, mapSource, facebookProfileIdFromUrl } from "@/lib/sources/helpers";
import {
  Table,
  TableBody,
  TableCell,
...
export default function SourceDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [srcRes, mediaRes] = await Promise.all([
        fetch(`${API_BASE}/api/v1/sources/${id}`),
        fetch(
          `${API_BASE}/api/v1/media?source_id=${id}&page_size=${discovery.media_page_size}`,
        ),
      ]);
      if (!srcRes.ok) throw new Error(`Source not found (${srcRes.status})`);
      const src = await srcRes.json();
      setSource(mapSource(src));
  ...
  const handleDiscover = async () => {
    ...
    try {
      ...
      setDiscoverMsg("Discovering content…");
      // Poll job until worker finishes — enqueue alone always returns new=0.
      let final: any = null;
      for (let i = 0; i < 90; i++) {
        ...
        if (i % 5 === 4) {
          setDiscoverMsg(`Discovering content… (${i * 2}s)`);
        }
      }
      ...
      const found = Number(result.total_found ?? result.discovered ?? 0);
      const neu = Number(result.new ?? 0);
      const autoQueued = Number(result.auto_transcribe_queued ?? 0);
      const autoSuffix =
        autoQueued > 0
          ? ` Auto-queued ${autoQueued} for transcription.`
          : source?.auto_transcribe
            ? " Auto-transcribe on — nothing new to queue (or at concurrency cap)."
            : "";
      const note = typeof result.note === "string" ? result.note : "";
      if (note.includes("stub")) {
        setDiscoverMsg(
          "Worker is still on the old stub — restart the Celery terminal, then Discover again."
        );
      } else if (neu > 0) {
        setDiscoverMsg(
          `Cataloged ${neu} item${neu !== 1 ? "s" : ""} (${found} found).${autoSuffix}`,
        );
      } else if (found > 0) {
        setDiscoverMsg(`Up to date — ${found} items found, nothing new.${autoSuffix}`);
      } else {
        setDiscoverMsg(`No items found.${autoSuffix}`);
      }
      await load();
    } catch (err: any) {
  ...
  const handleSaveSource = async (data: {
    ...
    if (!id || !data.source_url?.trim()) return;
    setDiscoverMsg(null);
    ...
    const entered = data.source_url.trim().replace(/\/+$/, "");
    const currentIdentity = (source?.source_url || "").replace(/\/+$/, "");
    const currentVanity = (source?.vanity_url || "").replace(/\/+$/, "");
    // Only send source_url when the user actually changed it — avoids Facebook
    // re-resolve on every metadata edit (and long/ hung PATCH failures).
    const urlChanged =
      normFb(entered) !== normFb(currentIdentity) &&
      (!currentVanity || normFb(entered) !== normFb(currentVanity));
    ...
    if (data.source_type) payload.source_type = data.source_type;
    if (urlChanged) payload.source_url = entered;

    const endpoint = `${window.location.origin}/api/v1/sources/${id}`;
    let res: Response;
    try {
      ...
      // Fallback if fetch is still patched/broken in this browser profile.
      res = await apiFetch(`/api/v1/sources/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
    ...
    try {
      ...
      throw new Error(
        `Failed to update source (${res.status}): ${raw.slice(0, 240) || res.statusText}`,
      );
    }
    if (!res.ok) {
      throw new Error(
        formatApiDetail(
          updated.detail,
          `Failed to update source (${res.status}): ${raw.slice(0, 240)}`,
        ),
      );
    }
    setSource(mapSource(updated));
    setDiscoverMsg("Source updated.");
    if (data.discover) {
  ...
  if (error || !source) {
    return (
      <div className="space-y-4 py-12">
        <Link href="/sources" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-3 min-w-0 flex-1">
          ...
          {(() => {
            const profileId = isFacebook
              ? facebookProfileIdFromUrl(source.source_url)
              : null;
            const idUrl = profileId
              ? `https://www.facebook.com/profile.php?id=${profileId}`
              : null;
            const vanity =
              source.vanity_url ||
              (isFacebook && !profileId ? source.source_url : null);
            const editBtn = (
              <button
                type="button"
                  ...
                  {vanity && (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <a
                        ...
                        title={vanity}
                      >
                        <span className="truncate max-w-[480px] font-mono">
                          {displaySourcePath(vanity)}
                        </span>
                      ...
                      <>
                        ...
                        >
                          <span className="truncate max-w-[420px] font-mono">
                            {displaySourcePath(idUrl)}
                          </span>
                          <ExternalLink className="w-3.5 h-3.5 shrink-0" />
                        </a>
                      </>
                    ) : (
                      <span className="text-sm text-amber-600">Not resolved</span>
                    )}
                    {editBtn}
                  </div>
                </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border/60 bg-muted/20 px-6 py-14 text-center space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            <Button variant="outline" onClick={handleDiscover} disabled={discovering} className="gap-2">
              {discovering ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
              Discover
            </Button>
          </div>
