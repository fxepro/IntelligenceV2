...
import {
  COURSE_SOURCE_TYPES,
  type CourseSourceFormData,
} from "@/components/sections/AddCourseSourceDialog";
import {
  CURRICULUM_TYPES,
  normalizeCurriculumType,
} from "@/lib/courses/curriculum-types";
import { libraryCourseIdFromSource, slugifyCourseId } from "@/lib/sources/helpers";
import type { Source } from "@/lib/mock-data/sources";

function curriculumUrlFromSource(source: Source): string {
  const url = source.source_url || "";
  if (url.startsWith("http://") || url.startsWith("https://")) return url;
  return source.vanity_url || "";
}

export function EditCourseSourceDialog({
  open,
  onClose,
  onSave,
  source,
  ...
  open: boolean;
  onClose: () => void;
  onSave: (data: CourseSourceFormData) => Promise<void>;
  source?: Source | null;
}) {
  ...
  const [curriculumType, setCurriculumType] = useState("manual");
  ...
  useEffect(() => {
    if (open && source) {
      setName(source.name || "");
      setCourseId(libraryCourseIdFromSource(source) || slugifyCourseId(source.name || ""));
      setUrl(curriculumUrlFromSource(source));
      setSourceType(source.source_type || "website");
      setCurriculumType(normalizeCurriculumType(source.connector));
      setError(null);
    }
  }, [open, source]);
    ...
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="sm:max-w-2xl gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50 space-y-2">
          <DialogTitle className="text-xl">Edit Course Source</DialogTitle>
          <DialogDescription className="text-sm leading-relaxed">
            Updates are saved to the source record in the database first.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Stored on the source record:{" "}
              <span className="font-mono">v2/data/{courseId || "…"}/</span>
            </p>
          </div>
